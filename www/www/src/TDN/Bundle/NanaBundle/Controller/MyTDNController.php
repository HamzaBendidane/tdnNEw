...
class MyTDNController extends Controller {
    ...
    *
    */
    public function ajouterHobbyAction ($action) {

        $variables = array();  

        if ($action === 'modifier') {
            $err = $this->_ajouterHobby();
    ...
    * _ajouterHobby
    *
    * Traitement des données pour l'ajout d'un nouveau « Hobby »
    *
    * @version 0.0.1
    *
    * @return boolean
    *
    */
    private function _ajouterHobby () {
        ...
        $form_hobby->handleRequest($request);
        ...
        if ($form_hobby->isValid()) {
            ...
            $this->get('session')->getFlashBag()->add('success', 'La passion a été ajoutée ');
            return true;
        } else {
            $this->get('session')->getFlashBag()->add('fail', 'Une erreur s’est produite');
            return false;
        }
    }
    ...
    */
    ...
    /**
    *
    * supprimerImageHobbyAction
    *
    * Supprimer une image de la galerie d'un Hobby
    *
    * @version 0.1.0
    *
    * @param interger $id — Identifiant de l'image
    *
    * @return Response
    *
    */ 
    public function supprimerImageHobbyAction($id) {
       ...
       // Récupération de l'entity manager qui va nous permettre de gérer les entités.
        $em = $this->get('doctrine.orm.entity_manager');
        $rep = $em->getRepository('TDN\Bundle\NanaBundle\Entity\NanaHobbyImageProxy');
        $proxy = $rep->find($id);
    ...
    *
    */ 
    public function supprimerImagePortraitAction ($id) {
    ...
    * showMyNanasAction
    *
    * Affichage du cercle des nanas sur TDN
    *
    * @version 0.0.1
    *
    * @return Response
    *
    **/
    public function showMyNanasAction () {

        $em = $this->get('doctrine.orm.entity_manager');
        $rep_nanas = $em->getRepository('TDN\Bundle\NanaBundle\Entity\Nana');
        $vars['me'] = $rep_nanas->find(3);
        
        // $vars['me']= $this->get('security.context')->getToken()->getUser();
        $vars['isAffiche'] = array('',$this->classeAffiche[1],'','');

        return $this->render('TDNNanaBundle:Partiels:myProfil_Network.html.twig', $vars);
    }
}